...
export const userService = {
  ...
  async getUserById(userId: number): Promise<User> {
    const response = await apiClient.get<User>(`${API_URL}/` + userId);
    return response.data;
  },

  //router.get("/ordered-suppliers", AuxiliarController.listOrderedSuppliers);
  async listOrderedSuppliers(): Promise<SupplierOrder[]> {
    const response = await apiClient.get<SupplierOrder[]>(`${API_BASE_URL}/auxiliar/ordered-suppliers`);
    return response.data;
  },

  async startQuarantine(userId: number): Promise<User> {
    const response = await apiClient.patch<User>(`${API_URL}/startQuarantine/` + userId);
    return response.data;
  },

  async endQuarantine(userId: number): Promise<User> {
    const response = await apiClient.patch<User>(`${API_URL}/endQuarantine/` + userId);
    return response.data;
  }
};
