...
import { performanceStyles } from './PerformanceScreen.styles';
...
export default function PerformanceScreen() {
  ...
  const [mealTypeFilter, setMealTypeFilter] = useState<string>("all"); // "all", "1" (almoço), "2" (jantar)
  const [dishTypeFilter, setDishTypeFilter] = useState<string>("all"); // "all", "fish", "meat", "vegetarian"
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (filterType === "day" && dayFilter) {
        ...
        // Intervalo de datas é obrigatório para filtro por dia da semana
      ...
      // Adicionar filtro de tipo de refeição (almoço/jantar) se não for "Todos"
      if (mealTypeFilter !== "all") {
        filters.mealTypeId = Number(mealTypeFilter); // 1 = Almoço, 2 = Jantar
      }

      // Adicionar filtro de tipo de prato se não for "Todos"
      if (dishTypeFilter !== "all") {
        // Mapear: dishTypeId 1 = Carne, dishTypeId 2 = Peixe, dishTypeId 3 = Vegetariano
        if (dishTypeFilter === "fish") {
          filters.dishTypeId = 2; // Peixe
        } else if (dishTypeFilter === "meat") {
          filters.dishTypeId = 1; // Carne
        } else if (dishTypeFilter === "vegetarian") {
          filters.dishTypeId = 3; // Vegetariano
        }
      }

      // Adicionar filtro por refeitório se o utilizador for RefectoryManager ou RefectoryStaff
      if ((user?.role === "RefectoryManager" || user?.role === "RefectoryStaff") && user?.refeitorioId) {
        filters.refeitorioId = user.refeitorioId;
      }

      // Adicionar filtro por cantina se o utilizador for CanteenManager
      if (user?.role === "CanteenManager" && user?.canteenId) {
        filters.canteenId = user.canteenId;
      }
      ...
      // Garantir que mealTypeId e dishTypeId sejam passados também para waste reports
      if (mealTypeFilter !== "all") {
        reportFilters.mealTypeId = Number(mealTypeFilter);
      }
      if (dishTypeFilter !== "all") {
        if (dishTypeFilter === "fish") {
          reportFilters.dishTypeId = 2; // Peixe
        } else if (dishTypeFilter === "meat") {
          reportFilters.dishTypeId = 1; // Carne
        } else if (dishTypeFilter === "vegetarian") {
          reportFilters.dishTypeId = 3; // Vegetariano
        }
      }
      // Garantir que refeitorioId seja passado também para waste reports
      if ((user?.role === "RefectoryManager" || user?.role === "RefectoryStaff") && user?.refeitorioId) {
        reportFilters.refeitorioId = user.refeitorioId;
      }
      if (user?.role === "CanteenManager" && user?.canteenId) {
        reportFilters.canteenId = user.canteenId;
      }
      const reportData = await wasteReportService.getWasteReportStatistics(reportFilters);
      console.log("Received waste report data:", reportData);
  ...
  }, [filterType, dayFilter, weekFilter, dayOfWeekFilter, dayOfWeekStartDate, dayOfWeekEndDate, dateRangeStart, dateRangeEnd, mealTypeFilter, dishTypeFilter, monthFilter, yearFilter, user?.role, user?.refeitorioId, user?.canteenId]);
  ...
  }, [showDayCalendar, showWeekCalendar, showDayOfWeekStartCalendar, showDayOfWeekEndCalendar]);

  // Buscar nome do refeitório ou cantina
  useEffect(() => {
    ...
    if (user) {
      if (user.role === "RefectoryManager") {
        fetchRefeitorioName();
      } else if (user.role === "CanteenManager") {
        fetchCanteenName();
      }
    }
  }, [user?.refeitorioId, user?.refeitorio?.name, user?.canteenId, user?.canteen?.name, user?.role, user]);
  ...
  const filteredReservationTotals = (() => {
    ...
    // Filtrar por tipo de refeição (sempre no frontend porque pode ser combinado com dishTypeFilter)
    if (mealTypeFilter !== "all") {
      filteredMeals = filteredMeals.filter(meal => meal.mealTypeId === Number(mealTypeFilter));
    }

    // Filtrar por tipo de prato
    // Nota: Se dishTypeFilter estiver ativo, o backend já filtra os dados, mas vamos filtrar novamente
    // no frontend para garantir que os dados estão corretos (caso o dishTypeId esteja presente)
    if (dishTypeFilter !== "all") {
      let targetDishTypeId: number;
      if (dishTypeFilter === "fish") {
        targetDishTypeId = 2; // Peixe
      } else if (dishTypeFilter === "meat") {
        targetDishTypeId = 1; // Carne
      } else if (dishTypeFilter === "vegetarian") {
        targetDishTypeId = 3; // Vegetariano
      } else {
        targetDishTypeId = 0;
      }
      filteredMeals = filteredMeals.filter(meal => {
        // Se dishTypeId não estiver presente, assumir que o backend já filtrou e incluir
        if (meal.dishTypeId === undefined || meal.dishTypeId === null) {
          return true; // Incluir se não tiver dishTypeId (assumir que backend já filtrou)
        }
        // Se dishTypeId estiver presente, filtrar por ele
  ...
  const hasRequiredFilters = 
    (filterType === "day" && dayFilter) ||
    (filterType === "week" && weekFilter) ||
    (filterType === "dayOfWeek" && dayOfWeekFilter !== null) ||
    (filterType === "month" && monthFilter) ||
    (filterType === "year" && yearFilter);

  // Calcular desperdício total combinado usando dados filtrados
  ...
  // Verificar se há dados de qualquer tipo (usando dados filtrados)
  ...
  const getPeriodLabel = () => {
    ...
    if (filterType === "dayOfWeek" && dayOfWeekFilter !== null) {
      ...
      } else if (dayOfWeekStartDate) {
        label += ` (a partir de ${formatDateShort(dayOfWeekStartDate)})`;
      } else if (dayOfWeekEndDate) {
        label += ` (até ${formatDateShort(dayOfWeekEndDate)})`;
      }
      return label;
    }
        ...
        <div style={performanceStyles.contentWrapper}>
          <div style={performanceStyles.titleSection}>
            ...
            <h2 style={performanceStyles.pageTitle}>
              ...
              Desempenho - Desperdício de Refeições
            </h2>
              ...
              {canteenName && user?.role === "CanteenManager" && (
                <div style={{
                  padding: '6px 12px',
                  backgroundColor: '#f3f4f6',
                  borderRadius: '6px',
            ...
            <p style={performanceStyles.pageDescription}>
              Visualize o desperdício de reservas não consumidas e desperdício de pratos por dia, mês ou ano.
            </p>
          </div>
            ...
            <div style={performanceStyles.noDataContainer}>
              <p>
                 {filterType === "day" && !dayFilter
                   ...
                   : "Não há dados de desperdício para o período selecionado."}
              </p>
            </div>
          ) : (
            <>
                             ...
                             .filter((mealData) => {
                               ...
                               if (dishTypeFilter !== "all") {
                                 ...
                                 if (mealData.dishTypeId === undefined || mealData.dishTypeId === null) {
                                   dishTypeMatch = true; // Incluir se não tiver dishTypeId (assumir que backend já filtrou)
                                 } else {
