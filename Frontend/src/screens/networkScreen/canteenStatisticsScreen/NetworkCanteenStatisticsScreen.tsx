...
import { canteenStatisticsStyles } from '../../canteenManagerScreen/statisticsScreen/CanteenStatisticsScreen.styles';
...
export default function NetworkCanteenStatisticsScreen() {
  const navigate = useNavigate();
  const user = useSelector((state: any) => state.auth.user);
  const userName = user ? user.name : "Utilizador";
  
  // Filtro de cantina (para NetworkManager pode escolher qualquer cantina)
  const [selectedCanteenId, setSelectedCanteenId] = useState<string>("all");
  const [canteens, setCanteens] = useState<Array<{ id: number; name: string }>>([]);
  ...
  // Buscar refeitórios quando uma cantina é selecionada
  useEffect(() => {
    const fetchRefeitorios = async () => {
      if (!selectedCanteenId || selectedCanteenId === "all") {
        setRefeitorios([]);
        return;
      }
      ...
      try {
        const response = await axios.get<Array<{ id: number; name: string }>>(
          `${API_BASE_URL}/canteens/${selectedCanteenId}/refeitorios`
        );
  ...
  // Atualizar nome da cantina selecionada
  useEffect(() => {
    if (selectedCanteenId && selectedCanteenId !== "all") {
      const canteen = canteens.find(c => c.id === Number(selectedCanteenId));
  ...
  const loadStatistics = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get<ProductionStatistics>(
        `${API_BASE_URL}/canteens/${selectedCanteenId}/production-statistics`,
        { params }
      );
  ...
  }, [selectedCanteenId, filterType, dayFilter, weekFilter, dayOfWeekFilter, dayOfWeekStartDate, dayOfWeekEndDate, dateRangeStart, dateRangeEnd, monthFilter, yearFilter, selectedRefeitorioId, selectedDishTypeId]);
  ...
  const loadIngredientsStatistics = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/canteens/${selectedCanteenId}/ingredients-statistics`,
        { params }
      );
        ...
        <div style={canteenStatisticsStyles.contentWrapper}>
          <div style={canteenStatisticsStyles.titleSection}>
            ...
            <p style={canteenStatisticsStyles.pageDescription}>
              Visualize os KPIs de produção das cantinas, incluindo pratos produzidos, quantidade de ingredientes utilizados e outros indicadores de produção. Selecione uma cantina para ver as estatísticas.
            </p>
          </div>

          {/* Filtros */}
          <div style={canteenStatisticsStyles.filtersContainer}>
            {/* Filtro de Cantina - PRIMEIRO */}
            <div style={canteenStatisticsStyles.filterGroup}>
              <label style={canteenStatisticsStyles.filterLabel} htmlFor="canteen-filter">Cantina:</label>
              <select
                id="canteen-filter"
              ...
              >
                <option value="all">Escolher Cantina</option>
            ...
            {filterType === "day" && (
              <div style={canteenStatisticsStyles.filterGroup}>
                ...
                <div style={{ position: 'relative', display: 'inline-block', width: '100%', maxWidth: '200px' }} data-calendar-container>
                  <div 
            ...
            {/* Filtro de Refeitório - só aparece se uma cantina estiver selecionada */}
            {selectedCanteenId && selectedCanteenId !== "all" && (
              <div style={canteenStatisticsStyles.filterGroup}>
          ...
          {/* Mensagem se nenhuma cantina estiver selecionada */}
          {selectedCanteenId === "all" && (
            <div style={canteenStatisticsStyles.noDataContainer}>
              <p>Por favor, selecione uma cantina para ver as estatísticas.</p>
            </div>
          )}
